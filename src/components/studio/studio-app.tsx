  ...
  async function runPrompt(text: string) {
    ...
    try {
      ...
      if ((err as Error).name === "AbortError") {
        ...
        const message =
          pay.message ||
          "Payment required: add Studio credits or connect a BYOK key to continue.";
        setCapacityNote(message);
        toast.error(message, {
          duration: 14000,
          action: {
        ...
        store.replaceMessage(conversationId, branchId, {
          ...assistant,
          content:
            "Mesh needs Studio credits or a BYOK key. Your prompt is saved — add funding, then Retry.",
          agents: tracesIdle(),
          verdict: "PENDING",
        });
        setTraces(tracesIdle());
