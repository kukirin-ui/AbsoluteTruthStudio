...
export function PluginDrawer({
  ...
  const items = useMemo(() => {
    return PLUGINS_CATALOG.filter((item) => {
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" title="Plugins" className="left-auto right-0 w-[min(100%,28rem)]">
        <p className="mb-3 text-sm text-muted">
          {PLUGIN_COUNTS.total} plugins — {PLUGIN_COUNTS.free} free, {PLUGIN_COUNTS.paid} paid.
          Attach any tool to the tabs it supports. Four specialized tabs stay in lockstep — agent swap per tab.
        </p>
        <div className="mb-4 flex flex-wrap gap-1">
          {(
            [
              ["all", `All ${PLUGIN_COUNTS.total}`],
              ["free", `Free ${PLUGIN_COUNTS.free}`],
              ["paid", `Paid ${PLUGIN_COUNTS.paid}`],
              ["on", "On tabs"],
            ] as const
          ).map(([id, label]) => (
            <Button
