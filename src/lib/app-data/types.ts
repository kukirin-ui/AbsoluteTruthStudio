...
/** Prefix used in `CallToolResult.errorMessage` while the preview awaits a token. */
export const CONNECTOR_TOKEN_PENDING_CODE = "connector_token_pending";

/** Window event name dispatched when the preview bridge receives a fresh token. */
...
export type CallToolResult<T = unknown> = {
  ...
  loginUrl?: string;
  /** Preview-only: token not yet (or no longer) available. */
  pending?: boolean;
};
